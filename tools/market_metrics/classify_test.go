...
func TestReplayForcedOpen_StopLoss(t *testing.T) {
	...
	pnl, exit, closed, hit := replayForcedOpen(k5, 0, "温和桶")
...
func TestReplayForcedOpen_Trailing(t *testing.T) {
	...
	pnl, exit, closed, hit := replayForcedOpen(k5, 0, "爆拉桶")
...
func TestReplayForcedOpen_MaxHoldAndHolding(t *testing.T) {
	...
	// 数据不足（只回放 2 根）→ 持有中
	k5s := k5[:2]
	_, exit, closed, _ = replayForcedOpen(k5s, 0, "爆拉桶")
